...
| Doc | Purpose |
|-----|---------|
| [ANGIES-ADMIN-AGENT-DIRECTION.md](./ANGIES-ADMIN-AGENT-DIRECTION.md) | **Owner direction** — clone homepage into admin, API back to same sources |
| [UNIFIED-ADMIN-PLAN.md](./UNIFIED-ADMIN-PLAN.md) | Implementation plan + MVP checklist |
...
Public order in [`HomeView.tsx`](../components/home/HomeView.tsx) (source of truth for the rebuild):

| # | Public section | Admin today | Target admin anchor |
|---|----------------|-------------|---------------------|
| 1 | Hero | Homepage tab | `#hero` |
| 2 | Prologue | Homepage tab | `#prologue` |
| 3 | Story | Homepage tab | `#story` |
| 4 | Menu | Menu tab (import only) | `#menu` + `#modifiers` |
| 5 | Locations (+ schedule block) | Locations + Schedule tabs | `#locations` + `#schedule` |
| 6 | Social | — | `#social` (site_settings) |
| 7 | Catering | — | `#catering` (site_settings) |
| 8 | FinalConversion | — | v2 optional |
| 9 | SiteFooter / order | Order drawer, Clover | out of scope for v1 |

**Agent principle:** [ANGIES-ADMIN-AGENT-DIRECTION.md](./ANGIES-ADMIN-AGENT-DIRECTION.md) — each public section gets an admin section that saves to the **same** API/table the homepage already reads.
...
Follow **[ANGIES-ADMIN-AGENT-DIRECTION.md](./ANGIES-ADMIN-AGENT-DIRECTION.md)** (owner spec) and **[UNIFIED-ADMIN-PLAN.md](./UNIFIED-ADMIN-PLAN.md)** (implementation):

- `SiteAdminClient` replaces tab shell; mirrors `HomeView` order
- `ImageAttachField` on hero, story, menu (no URL typing)
- Relational `PATCH /api/admin/catalog-menu/*` for items + modifiers + meat prices
- Social + catering in `site_settings`
- JSON import under Advanced only
- 15-step MVP checklist in agent direction doc
