...
import { dbUpdateScheduleItem, dbDeleteScheduleItem } from "@/lib/catalog-db/schedule-db";
...
export async function PATCH(
  ...
  const parsed = parseScheduleItemForAdmin(body, { idOptional: false });
  ...
  try {
    const updated = await dbUpdateScheduleItem(id, parsed.item);
    if (!updated) return NextResponse.json({ ok: false, error: "Not found" }, { status: 404 });
    revalidatePublicCatalog();
    return NextResponse.json({ ok: true, item: updated });
  } catch (e) {
    console.error("[admin/schedule PATCH]", e);
...
export async function DELETE(
  ...
  if (!id?.trim()) return NextResponse.json({ ok: false, error: "Invalid id" }, { status: 400 });
  try {
    const deleted = await dbDeleteScheduleItem(id.trim());
    if (!deleted) return NextResponse.json({ ok: false, error: "Not found" }, { status: 404 });
    revalidatePublicCatalog();
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error("[admin/schedule DELETE]", e);
