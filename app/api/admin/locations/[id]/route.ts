...
import { dbUpdateLocationItem, dbDeleteLocationItem } from "@/lib/catalog-db/location-db";
...
export async function PATCH(
  ...
  const parsed = parseLocationItemForAdmin(body, { idOptional: false });
  ...
  try {
    const updated = await dbUpdateLocationItem(id, parsed.item);
    if (!updated) return NextResponse.json({ ok: false, error: "Not found" }, { status: 404 });
    revalidatePublicCatalog();
    return NextResponse.json({ ok: true, item: updated });
  } catch (e) {
    console.error("[admin/locations PATCH]", e);
...
export async function DELETE(
  ...
  if (!id?.trim()) return NextResponse.json({ ok: false, error: "Invalid id" }, { status: 400 });
  try {
    const deleted = await dbDeleteLocationItem(id.trim());
