...
import { SquareCardFields } from "@/components/order/SquareCardFields";
...
export function OrderDrawer() {
  ...
  // Square card step — mount SquareCardFields only while active (clean unmount destroys iframe)
  ...
  const handleSquareToken = useCallback(
    async (token: string) => {
      setSquareToken(token);
      await submitOrder(token);
    },
    [setSquareToken, submitOrder],
  );
  ...
  // Reset card step when drawer closes or order confirms
  useEffect(() => {
    if (!orderDrawerOpen || confirmationId) {
      setShowCardFields(false);
    }
  }, [confirmationId, orderDrawerOpen]);
            ...
            <footer className="shrink-0 space-y-2 border-t border-white/10 p-3 pb-[max(1rem,env(safe-area-inset-bottom))] sm:space-y-3 sm:p-4 sm:pb-4">
              {cartHasUnpricedItems ? (
                ...
                <SquareCardFields
                  ...
                  externalBusy={orderStatus === "submitting"}
                />
              )}
            </footer>
              </>
